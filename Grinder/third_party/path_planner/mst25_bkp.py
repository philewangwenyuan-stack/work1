...
def export_path_to_json(path, filename='path_output.json', output_dir=None):
    ...
    path_data = {
        "metadata": {
            ...
            "description": "Boustrophedon path planning with boundary fix",
            "version": "3.2-fixed",
            "fix_notes": "Fixed: boundary overflow (radius=0.0), strict check, post-filter, param logging"
        },
        "path": []
    }
    ...
    path_data["statistics"]["total_distance"] = round(total_distance, 2)
    path_data["statistics"]["average_point_spacing"] = round(
        total_distance / (len(path) - 1) if len(path) > 1 else 0, 3
    )
